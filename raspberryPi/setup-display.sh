...
# ---------- Output auto-detect ----------
# Returns a Wayfire/wlroots output name like HDMI-A-1 or DP-1.
detect_output_name() {
  local prefer="${KIOSK_OUTPUT_PREFER^^}"

  # Read connected DRM connectors from sysfs:
  # e.g. /sys/class/drm/card1-HDMI-A-1/status -> "connected"
  ...
  # Helper: strip "cardX-" prefix -> connector name
  local c
  local conn=()
  ...
  # Prefer HDMI, then DP, then anything
  if [[ "$prefer" == "HDMI" ]]; then
